...
import com.henu.example.com.wechat.main.Serices.GroupService;
import com.henu.example.com.wechat.main.WXServer;
import com.henu.example.com.wechat.main.db.ACache;
...
public class GroupUitls {
    ...
    public void updateGroupName(String groupId, String groupName, final CallBack callBack) {
        ...
        OkHttpManager.getInstance().post(params, WXServer.URL_UPDATE_Groupnanme, new OkHttpManager.HttpCallBack() {
            @Override
    ...
    public void addMembersToGroup(final String groupId, List<String> memebers, final List<String> exitingMembers, final CallBack callBack) {
        ...
        OkHttpManager.getInstance().post(params, WXServer.URL_GROUP_ADD_MEMBERS, new OkHttpManager.HttpCallBack() {
            @Override
            public void onResponse(JSONObject jsonObject) {
                int code = jsonObject.getIntValue("code");
                if (code == 1000) {
            ...
            public void run() {
                ...
                if (exitingMembers.size() < 9) {
                    ...
                    for (int i = 0; i < add; i++) {
                        ...
                        EaseUser easeUser = Helper.getInstance().getContactList().get(hxid);
    ...
    public void getGroupMembersInServer(final String groupId, final String groupName, final MembersCallBack membersCallBack) {
        ...
        List<Param> params = new ArrayList<>();
        params.add(new Param("groupId", groupId));
        OkHttpManager.getInstance().post(params, WXServer.URL_GROUP_MEMBERS, new OkHttpManager.HttpCallBack() {
            @Override
            public void onResponse(JSONObject jsonObject) {
